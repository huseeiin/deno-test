...
export async function getVideos() {
  ...
  await new HTMLRewriter()
    .on(".full-row-thumbs li", {
      element(el) {
        currentVideo = { vkey: el.getAttribute("data-video-vkey")! };
      },
    })
    .on(".full-row-thumbs li .thumbnailTitle", {
      text(el) {
        if (!el.lastInTextNode) {
          currentVideo.title = decode(el.text.trim());

          videos.push(currentVideo);
        }
      },
    })
    .on(".full-row-thumbs li img", {
      element(el) {
        currentVideo.thumb = el.getAttribute("src");
      },
    })
    .on(".full-row-thumbs li .views", {
      text(el) {
        if (!el.lastInTextNode) currentVideo.views = el.text;
      },
    })
    .on(".full-row-thumbs li .duration", {
      text(el) {
...
export async function getVideo(viewkey: string) {
  let thumb = "";
  ...
  await new HTMLRewriter()
    ...
    .on(".userInfo a", {
      element(e) {
        username = e.getAttribute("href")?.split("/")[2];
      },
    })
    .on(".userInfo a", {
      text(e) {
        if (!e.lastInTextNode) name = e.text;
      },
    })
    .on(".userInfo span", {
      text(t) {
        if (t.text.includes("Subscribers")) subscribers = t.text.split(" ")[0];
        if (t.text.includes("Videos")) videos = t.text.split(" ")[0];
      },
    })
    .on(".userAvatar img", {
      element(el) {
  ...
  } = JSON.parse(script) as Root;

  const { userInteractionCount: views } = interactionStatistic.find(
    (i) => i.interactionType === "http://schema.org/WatchAction",
  )!;

  const { userInteractionCount: likes } = interactionStatistic.find(
    (i) => i.interactionType === "http://schema.org/LikeAction",
  )!;
